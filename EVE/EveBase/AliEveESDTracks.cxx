...
fDrawNoRefit(false)
{
    ...
    fStyle        = settings.GetValue("tracks.lineStyle",1);
...
TEveTrackList* AliEveESDTracks::ITStracks()
{
    ...
    SetupPropagator(cont->GetPropagator(),
                    0.1*esd->GetMagneticField(), settings.GetValue("tracks.ITStrack.trackDepth",85));
    cont->GetPropagator()->SetMaxR(settings.GetValue("tracks.ITStrack.trackDepth",85));
    cont->SetLineWidth(fWidth);
...
TEveElementList* AliEveESDTracks::ByCategory()
{
    ...
    const Int_t   nCont = 9;
    const Float_t maxR  = settings.GetValue("tracks.full.trackDepth",530);
...
TEveElementList* AliEveESDTracks::ByType()
{
    ...
    const Float_t maxR  = settings.GetValue("tracks.byType.animate",false) ? 0 : settings.GetValue("tracks.Full.trackDepth",520);
    const Float_t magF  = 0.1*esd->GetMagneticField();
    ...
    for (Int_t n = 0; n < esd->GetNumberOfTracks(); ++n)
    {
        ...
        {
            pid = at->GetPID();
            if (settings.GetValue("exotics.Selection",0)){
              // find really weird candidates
              if (at->GetTPCsignal() > settings.GetValue("exotics.TPCSignalMin",270) &&
                  at->GetITSsignal() > settings.GetValue("exotics.ITSSignalMin",160) &&
                  at->P() > settings.GetValue("exotics.PMin",10.)){
                pid = 14;
              }
            }
...
TEveElementList* AliEveESDTracks::ByPt()
{
    ...
    const Int_t   nCont = 6;
    const Float_t maxR  = settings.GetValue("tracks.Full.trackDepth",520);
    const Float_t magF  = 0.1*esd->GetMagneticField();
