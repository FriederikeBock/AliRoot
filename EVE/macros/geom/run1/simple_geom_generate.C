...
void simple_geom_generate(char *detectorName="", int runNumber=0, int year = 0)
{
    ...
    for(int i=0;i<detectorsList.size();i++)
    {
        ...
        TString fileNameGeomConfig = Form("geom_list_%s.txt",currentDetector);
        if (year != 0)
            fileNameGeomConfig = Form("geom_list_%s_%d.txt",currentDetector,year);
        ifstream in(fileNameGeomConfig.Data(), ios::in);
        cout<<"Adding shapes from file:"<<fileNameGeomConfig.Data()<<endl;
        ...
        {
            eve_tnode->SaveExtract(Form("../../../resources/geometry/run1/simple_geom_%s.root",currentDetector),
